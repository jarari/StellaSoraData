local EquipmentInstanceEditor = class("EquipmentInstanceEditor")
...
function EquipmentInstanceEditor:Init(parent,floorId,tbChar, tbDisc, tbNote)
    self.parent = parent
    ...
    CS.AdventureModuleHelper.EnterEquipmentInstanceMap(floorId,self.tbCharId)
    NovaAPI.EnterModule("AdventureModuleScene", true)
end
function EquipmentInstanceEditor:OnEvent_LoadLevelRefresh()
    self:ChangeNote()
    self:ResetNoteInfo()
end
function EquipmentInstanceEditor:OnEvent_LevelResult(bSuccess,nTime)
    if bSuccess then
...
function EquipmentInstanceEditor:OnEvent_AbandonBattle()
    self:OnEvent_LevelResult(false,0)
end
function EquipmentInstanceEditor:OnEvent_AdventureModuleEnter()
...
function EquipmentInstanceEditor:BindEvent()
...
function EquipmentInstanceEditor:UnBindEvent()
    if type(mapEventConfig) ~= "table" then
        return
    end
    for nEventId, sCallbackName in pairs(mapEventConfig) do
...
function EquipmentInstanceEditor:PlaySuccessPerform(FirstRewardItems,RewardItems,nStar)
...
function EquipmentInstanceEditor:SetTempActorAttribute(nCharId)
...
function EquipmentInstanceEditor:SetCharFixedAttribute()
...
function EquipmentInstanceEditor:CalCharFixedEffect(nCharId,bMainChar)
    local stActorInfo = CS.Lua2CSharpInfo_CharAttribute()
...
function EquipmentInstanceEditor:SetTheme()
    safe_call_cs_func(CS.AdventureModuleHelper.SetRglTheme,self.tbTheme)
end

function EquipmentInstanceEditor:ResetDiscInfo()
    local tbDiscInfo = {}
...
function EquipmentInstanceEditor:ResetNoteInfo()
    local tbNoteInfo = {}
...
function EquipmentInstanceEditor:ChangeNote()
...
function EquipmentInstanceEditor:OnEvent_TestBedNoteChange(noteList)
    if noteList then
        self.tbNote = {}
...
return EquipmentInstanceEditor