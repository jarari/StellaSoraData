local RegionBossBattleLevelEditor = class("RegionBossBattleLevelEditor")
...
function RegionBossBattleLevelEditor:Init(parent,nLevelId,tbChar, tbDisc, tbNote)
    self.parent = parent
    ...
    CS.AdventureModuleHelper.EnterRogueBossMap(ConfigTable.GetData("RegionBossLevel", nLevelId).FloorId,self.tbCharId,nLevelId)
    NovaAPI.EnterModule("AdventureModuleScene", true)
end
function RegionBossBattleLevelEditor:OnEvent_LoadLevelRefresh()
    self:ChangeNote()
    self:ResetNoteInfo()
end
function RegionBossBattleLevelEditor:OnEvent_LevelResult(bSuccess,nTime)
    if bSuccess then
...
function RegionBossBattleLevelEditor:OnEvent_AbandonBattle()
    self:OnEvent_LevelResult(false,0)
end
function RegionBossBattleLevelEditor:OnEvent_AdventureModuleEnter()
...
function RegionBossBattleLevelEditor:BindEvent()
...
function RegionBossBattleLevelEditor:UnBindEvent()
    if type(mapEventConfig) ~= "table" then
        return
    end
    for nEventId, sCallbackName in pairs(mapEventConfig) do
...
function RegionBossBattleLevelEditor:PlaySuccessPerform(FirstRewardItems,RewardItems,nStar)
...
function RegionBossBattleLevelEditor:SetTempActorAttribute(nCharId)
...
function RegionBossBattleLevelEditor:SetCharFixedAttribute()
...
function RegionBossBattleLevelEditor:CalCharFixedEffect(nCharId,bMainChar)
    local stActorInfo = CS.Lua2CSharpInfo_CharAttribute()
...
function RegionBossBattleLevelEditor:SetTheme()
    safe_call_cs_func(CS.AdventureModuleHelper.SetRglTheme,self.tbTheme)
end

function RegionBossBattleLevelEditor:ResetDiscInfo()
    local tbDiscInfo = {}
...
function RegionBossBattleLevelEditor:ResetNoteInfo()
    local tbNoteInfo = {}
...
function RegionBossBattleLevelEditor:ChangeNote()
...
function RegionBossBattleLevelEditor:OnEvent_TestBedNoteChange(noteList)
    if noteList then
        self.tbNote = {}
...
return RegionBossBattleLevelEditor