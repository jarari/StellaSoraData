local DailyInstanceEditor = class("DailyInstanceEditor")
...
function DailyInstanceEditor:Init(parent,floorId,tbChar, tbDisc, tbNote)
    self.parent = parent
    ...
    CS.AdventureModuleHelper.EnterDailyInstanceMap(floorId,self.tbCharId)
    NovaAPI.EnterModule("AdventureModuleScene", true)
end
function DailyInstanceEditor:OnEvent_LoadLevelRefresh()
    self:ChangeNote()
    self:ResetNoteInfo()
end
function DailyInstanceEditor:OnEvent_LevelResult(bSuccess,nTime)
    if bSuccess then
...
function DailyInstanceEditor:OnEvent_AbandonBattle()
    self:OnEvent_LevelResult(false,0)
end
function DailyInstanceEditor:OnEvent_AdventureModuleEnter()
...
function DailyInstanceEditor:BindEvent()
...
function DailyInstanceEditor:UnBindEvent()
    if type(mapEventConfig) ~= "table" then
        return
    end
    for nEventId, sCallbackName in pairs(mapEventConfig) do
...
function DailyInstanceEditor:PlaySuccessPerform(FirstRewardItems,RewardItems,nStar)
...
function DailyInstanceEditor:SetTempActorAttribute(nCharId)
...
function DailyInstanceEditor:SetCharFixedAttribute()
...
function DailyInstanceEditor.CalCharFixedAttr(nCharId,bMainChar)
    local stActorInfo = CS.Lua2CSharpInfo_CharAttribute()
    local nHeartStoneLevel = PlayerData.Char:CalCharacterAttrBattle(nCharId,stActorInfo,bMainChar)
    return stActorInfo,nHeartStoneLevel
end
function DailyInstanceEditor:SetTheme()
    safe_call_cs_func(CS.AdventureModuleHelper.SetRglTheme,self.tbTheme)
end
function DailyInstanceEditor:CalCharFixedEffect(nCharId,bMainChar)
    local stActorInfo = CS.Lua2CSharpInfo_CharAttribute()
    local nHeartStoneLevel = PlayerData.Char:CalCharacterAttrBattle(nCharId,stActorInfo,bMainChar,self.tbDisc)
    return stActorInfo,nHeartStoneLevel
end

function DailyInstanceEditor:ResetDiscInfo()
    local tbDiscInfo = {}
...
function DailyInstanceEditor:ResetNoteInfo()
    local tbNoteInfo = {}
...
function DailyInstanceEditor:ChangeNote()
...
function DailyInstanceEditor:OnEvent_TestBedNoteChange(noteList)
    if noteList then
        self.tbNote = {}
