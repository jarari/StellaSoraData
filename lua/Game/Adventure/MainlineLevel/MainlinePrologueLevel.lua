...
local PrologueSceneId = 1200103 -- 序章Id
local tbAfterDesertFloor = {1200104,1200106}
local sAfterBattleAvg = "ST1001"
local tbTrialCharacter = {{1200201,1200203,1200202},{1200201,1200203,1200202}}
local tbSelectPerks = {{500501,500801,500101}}
local mapBoxReward = {[1] = {500501,500801,500101}}
...
---------
    if Settings.bJumpPrologue then
        self.bDesertComplete = true
        self.bBattleEnd = true
        sAfterBattleAvg = "AA9999"
    end
...
function MainlinePrologueLevel:GetRewardNpc(nNpcId,nNpcUid)
    ...
    local mapBag = {mapPerk = {},mapItem = {},mapSlotPerk = {[1] = {nPerkId = 0,nQty = 0,nMaxLevel = 0},[2] = {nPerkId = 0,nQty = 0,nMaxLevel = 0},[3] = {nPerkId = 0,nQty = 0,nMaxLevel = 0},[4] = {nPerkId = 0,nQty = 0,nMaxLevel = 0}}}

    EventManager.Add(EventId.SelectPerk,self,SelectPerkCallBack)
    EventManager.Hit(EventId.OpenPanel, PanelId.FRThemePerkSelect, {{Perks = tbPerks,Theme = 0}}, 0, mapBag, {}, true, true)
end
...
function MainlinePrologueLevel:OnEvent_LevelStateChanged()
    self.bDesertComplete = true
    if #tbAfterDesertFloor >0 and #tbAfterDesertFloor > self.curFloorIdx and not self.bBattleEnd then
        self:ChangeFloor()
        return
    end
    ...
    local function UnloadCallback()
        ...
        if sAfterBattleAvg ~= "" then
            self.bBattleEnd = true
            self:SetLocalData()
            EventManager.Add("StoryDialog_DialogEnd",self,func_AvgEnd)
            EventManager.Hit("StoryDialog_DialogStart", sAfterBattleAvg,nil,self.nCurAvgCmdIdx)
        else
            func_AvgEnd()
        end
    end
...
function MainlinePrologueLevel:OnEvent_PrologueBattleArchive(isThreePlayer, revivalPoint, btnNames,nCmdId,nBoxId)
    ...
    self.nBoxId = nBoxId
    self.isThreePlayer = isThreePlayer == nil and false or isThreePlayer
    self.revivalPoint = revivalPoint  == nil and Vector3.zero or revivalPoint
    btnNames = btnNames  == nil and  {[0] = "",Count = 1} or btnNames
