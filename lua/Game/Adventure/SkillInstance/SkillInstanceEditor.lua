local SkillInstanceEditor = class("SkillInstanceEditor")
...
function SkillInstanceEditor:Init(parent,floorId,tbChar, tbDisc, tbNote)
    self.parent = parent
    ...
    CS.AdventureModuleHelper.EnterSkillInstanceMap(floorId,self.tbCharId)
    NovaAPI.EnterModule("AdventureModuleScene", true)
end
function SkillInstanceEditor:OnEvent_LoadLevelRefresh()
    self:ChangeNote()
    self:ResetNoteInfo()
end
function SkillInstanceEditor:OnEvent_LevelResult(bSuccess,nTime)
    if bSuccess then
...
function SkillInstanceEditor:OnEvent_AbandonBattle()
    self:OnEvent_LevelResult(false,0)
end
function SkillInstanceEditor:OnEvent_AdventureModuleEnter()
...
function SkillInstanceEditor:BindEvent()
...
function SkillInstanceEditor:UnBindEvent()
    if type(mapEventConfig) ~= "table" then
        return
    end
    for nEventId, sCallbackName in pairs(mapEventConfig) do
...
function SkillInstanceEditor:PlaySuccessPerform(FirstRewardItems,RewardItems,nStar)
...
function SkillInstanceEditor:SetTempActorAttribute(nCharId)
...
function SkillInstanceEditor:SetCharFixedAttribute()
...
function SkillInstanceEditor:CalCharFixedEffect(nCharId,bMainChar)
    local stActorInfo = CS.Lua2CSharpInfo_CharAttribute()
...
function SkillInstanceEditor:SetTheme()
    safe_call_cs_func(CS.AdventureModuleHelper.SetRglTheme,self.tbTheme)
end

function SkillInstanceEditor:ResetDiscInfo()
    local tbDiscInfo = {}
...
function SkillInstanceEditor:ResetNoteInfo()
    local tbNoteInfo = {}
...
function SkillInstanceEditor:ChangeNote()
...
function SkillInstanceEditor:OnEvent_TestBedNoteChange(noteList)
    if noteList then
        self.tbNote = {}
